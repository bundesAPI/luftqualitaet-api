"""
    Umweltbundesamt Air Data API

    Air data API of Umweltbundesamt  # noqa: E501

    The version of the OpenAPI document: 2.0.1
    Contact: immission@uba.de
    Generated by: https://openapi-generator.tech
"""


import sys
import unittest

from deutschland.luftqualitaet.model.airquality_date_start_cet3_and_following import (
    AirqualityDateStartCET3AndFollowing,
)

from deutschland import luftqualitaet

globals()["AirqualityDateStartCET3AndFollowing"] = AirqualityDateStartCET3AndFollowing
from deutschland.luftqualitaet.model.airquality_date_start_cet import (
    AirqualityDateStartCET,
)


class TestAirqualityDateStartCET(unittest.TestCase):
    """AirqualityDateStartCET unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def testAirqualityDateStartCET(self):
        """Test AirqualityDateStartCET"""
        # FIXME: construct object with mandatory attributes with example values
        # model = AirqualityDateStartCET()  # noqa: E501
        pass


if __name__ == "__main__":
    unittest.main()
